...
`MinValueValidator(0)` sur `Set.reps`.

Le test `test_zero_repetition_est_refuse` contredisait une decision produit deja prise. Il est reecrit en `test_un_nombre_de_repetitions_negatif_est_refuse`, et double d'un `test_zero_repetition_reste_accepte` qui verifie que la borne ne mord pas sur 0.
